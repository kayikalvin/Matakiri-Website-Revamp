...
const SectionHeader = ({ eyebrow, title, description }) => (
  <div className="text-center max-w-2xl mx-auto mb-12">
    ...
    {description && (
      <p className="text-ink-500 text-sm mt-3">{description}</p>
    )}
  </div>
);
...
const FocusCard = ({ icon: Icon, title, description, accent }) => {
  const borderColor = accent === 'laterite' ? 'border-laterite-500' : 'border-maize-400';
  return (
    <div className={`border ${borderColor} bg-white p-6 space-y-4`}>
      <div className={`h-10 w-10 flex items-center justify-center ${accent === 'laterite' ? 'text-laterite-500' : 'text-maize-500'}`}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
        ...
        <section className="py-20 md:py-28 bg-white">
          <div className="container mx-auto px-4">
            ...
            />
            {loading ? (
              <div className="text-center text-ink-500 font-mono text-sm">Loading projects…</div>
            ) : featuredProjects.length === 0 ? (
              <div className="text-center text-ink-500 font-mono text-sm">No projects yet.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
                {featuredProjects.map((project) => (
                  <Link
                    ...
                    className="group border border-border bg-parchment-50 hover:border-laterite-500/40 transition-colors overflow-hidden"
                  >
                    <div className="aspect-video bg-soil-900/5 flex items-center justify-center">
                      {project.images?.[0] ? (
                        <img
                        ...
                        <span className="text-4xl opacity-20">{project.title?.[0] || '?'}</span>
                      )}
                    </div>
                    <div className="p-5 space-y-2">
                      <h3 className="font-display text-lg font-medium text-ink-800 group-hover:text-laterite-600 transition-colors">
          ...
          <section className="py-16 bg-parchment-50 border-y border-border">
            <div className="container mx-auto px-4">
              ...
              <div className="flex flex-wrap justify-center items-center gap-8 opacity-60">
                {partners.map((p) => (
                  <div key={p._id || p.id} className="h-12">
                    {p.logo ? (
                      <img src={p.logo} alt={p.name} className="h-full object-contain" />
                    ) : (
                      <span className="font-mono text-xs text-ink-500">{p.name}</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        <section className="py-20 md:py-28 bg-white">
          <div className="container mx-auto px-4">
            ...
            />
            {loading ? (
              <div className="text-center text-ink-500 font-mono text-sm">Loading news…</div>
            ) : news.length === 0 ? (
              <div className="text-center text-ink-500 font-mono text-sm">No articles yet.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
                {news.map((article) => (
                  <Link
                  ...
                  >
                    ...
                    <p className="text-ink-500 text-sm line-clamp-2">{article.excerpt || article.description}</p>
        ...
        <section className="py-24 md:py-32 bg-soil-900 text-parchment-50">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-parchment-100/70 max-w-xl mx-auto text-sm mb-8">
              Whether you’re a donor, an NGO, a university, or a community member — there’s a place for you here.
            </p>
